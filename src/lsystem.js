    ...
    step() {
        ...
        this.rules.forEach((rule) => {
            ...
            while (last_pos < searchString.length && loopIterations < MAX_ITERATIONS) {
                ...
                if (pos_relative === -1)
                    break; // No more matches for this rule in the rest of the string
...
function applyAttributeArithmetic(attribute, rule) {
    if (!rule.attributeArithmetic)
        return attribute; // No arithmetic rule defined
    let type = "=";
    ...
    if (aaParts.length < 2)
        return attribute; // Cannot parse arithmetic rule
    ...
    if (attributeValueParts.length < 2)
        return attribute; // Cannot parse attribute
    ...
    if (isNaN(paramValue) || isNaN(arithmeticValue))
        return attribute; // Values are not numbers
    let resultValue;
    if (type === "=") {
...
function matchAttributeConditional(attribute, conditional) {
    if (!conditional)
        return true; // No conditional means always match
    const attributeParts = attribute.split("=");
    if (attributeParts.length < 2)
        return false; // Attribute not in expected format
    ...
    if (conditionalParts.length < 2)
        return false; // Conditional not in expected format
    ...
    if (isNaN(attributeValue) || isNaN(conditionalValue))
        return false; // Values are not numbers
    if (conditionalType === ">") {
